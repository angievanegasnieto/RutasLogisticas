...
public class ReportesController {
    ...
    /**
     * 📊 Obtener reporte de TODOS los pedidos en formato JSON
     */
    @GetMapping("/pedidos")
    public ResponseEntity<?> obtenerReportePedidosJSON(
        ...
        try {
            var reporte = reportesService.generarReportePedidos(fechaInicio, fechaFin, estado, conductorId);
            return ResponseEntity.ok(reporte);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Error al generar reporte: " + e.getMessage());
        }
    ...
    public ResponseEntity<byte[]> descargarPedidosCSV(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fechaInicio,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fechaFin,
            @RequestParam(required = false) String estado,
            @RequestParam(required = false) Long conductorId) {
        ...
        try {
            byte[] csvData = reportesService.generarReportePedidosCSV(fechaInicio, fechaFin, estado, conductorId);
    ...
    /**
     * 📥 Descargar reporte de pedidos en formato Excel
     */
    @GetMapping("/pedidos/excel")
    public ResponseEntity<byte[]> descargarPedidosExcel(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fechaInicio,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fechaFin,
            @RequestParam(required = false) String estado,
            @RequestParam(required = false) Long conductorId) {
        ...
        try {
            byte[] excelData = reportesService.generarReportePedidosExcel(fechaInicio, fechaFin, estado, conductorId);
