...
public class PedidosController {
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<PedidoView> actualizarPedido(
            @PathVariable Long id,
            @RequestBody CreatePedidoRequest request) {
        try {
            PedidoView pedidoActualizado = pedidoService.actualizarPedido(id, request);
    ...
    /**
     * 🔄 ACTUALIZAR PEDIDO PARCIALMENTE (para reintentos con reprogramación)
     * Permite actualizar fecha_programada, ventana_inicio, ventana_fin y estado
     */
    @PatchMapping("/{id}/parcial")
    public ResponseEntity<PedidoView> actualizarPedidoParcial(
            @PathVariable Long id,
            @RequestBody Map<String, Object> cambios) {
        try {
            PedidoView pedidoActualizado = pedidoService.actualizarPedidoParcial(id, cambios);
            return ResponseEntity.ok(pedidoActualizado);
        } catch (Exception e) {
            return ResponseEntity.badRequest().build();
        }
    ...
    public ResponseEntity<PedidoView> asignarConductor(
            @PathVariable Long id,
            @RequestParam Long conductorId) {
        try {
            PedidoView pedidoActualizado = pedidoService.asignarConductor(id, conductorId);
            return ResponseEntity.ok(pedidoActualizado);
        } catch (Exception e) {
            return ResponseEntity.badRequest().build();
        }
